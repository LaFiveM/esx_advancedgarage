...
function OpenMenuGarage(PointType)
	...
	elseif PointType == 'sherifing_pound_point' then
		table.insert(elements, {label = _U('return_owned_sherifing').." ($"..Config.SherifingPoundPrice..")", value = 'return_owned_sherifing'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'garage_menu', {
		...
		elseif action == 'return_owned_sherifing' then
			ReturnOwnedSherifingMenu()
...
function ListOwnedCarsMenu()
	...
	local isPrivate = ((this_Garage.Private and has_value(userProperties, this_Garage.Private)) or false)
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedCars', function(ownedCars)
		if #ownedCars == 0 then
			...
			exports['mythic_notify']:DoHudText('inform', _U('garage_nocars'), 3000, { ['background-color'] = '#22b1e6', ['color'] = '#ffffff' })
		else
			for _,v in pairs(ownedCars) do
				if v.garage and (this_Garage.Private == v.garage or this_Garage.name == v.garage)  or (v.garage == nil or v.garage == "") and this_Garage.name and this_Garage.name == "main" then

					--if not isPrivate or (isPrivate and this_Garage.Private and this_Garage.Private == v.garage) then
			...
			if #elements > 0 and this_Garage.PreviewCar ~= nil then
	
				HandleCamera(true)
	
				SpawnLocalVehicle(elements[1].value.vehicle)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_car', {
				...
				if data.current.value.stored then
					...
					--TriggerServerEvent('bb256980-aac3-4b39-afea-0bed0ad3d9d9', data.current.value.plate, "")
	
					SpawnVehicle(data.current.value.vehicle, data.current.value.plate, data.current.value.gps)
	
				else
	
					--ESX.ShowNotification(_U('car_is_impounded'))
					exports['mythic_notify']:DoHudText('inform', _U('car_is_impounded'), 3000, { ['background-color'] = '#22b1e6', ['color'] = '#ffffff' })
				...
				if this_Garage.PreviewCar ~= nil then
	
					SpawnLocalVehicle(data.current.value.vehicle)
...
function ListOwnedBoatsMenu()
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedBoats', function(ownedBoats)
		if #ownedBoats == 0 then
			...
			exports['mythic_notify']:DoHudText('inform', _U('garage_noboats'), 3000, { ['background-color'] = '#22b1e6', ['color'] = '#ffffff' })
		else
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_boat', {
			...
			if data.current.value.stored then
				...
				exports['mythic_notify']:DoHudText('inform', _U('boat_is_impounded'), 3000, { ['background-color'] = '#22b1e6', ['color'] = '#ffffff' })
			end
		end, function(data, menu)
			menu.close()
		end)
...
function ListOwnedAircraftsMenu()
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedAircrafts', function(ownedAircrafts)
		if #ownedAircrafts == 0 then
			...
			exports['mythic_notify']:DoHudText('inform', _U('garage_noaircrafts'), 3000, { ['background-color'] = '#22b1e6', ['color'] = '#ffffff' })
		else
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_aircraft', {
			...
			if data.current.value.stored then
				...
				exports['mythic_notify']:DoHudText('inform', _U('aircraft_is_impounded'), 3000, { ['background-color'] = '#22b1e6', ['color'] = '#ffffff' })
			end
		end, function(data, menu)
			menu.close()
		end)
			...
			if valid then
				if engineHealth < 0 then
					if Config.UseDamageMult then
						local apprasial = math.floor((0 - engineHealth)/0 *Config.CarPoundPrice*Config.DamageMult)
						RepairVehicle(apprasial, vehicle, vehicleProps)
					else
						local apprasial = math.floor((0 - engineHealth)/0 *Config.CarPoundPrice)
						RepairVehicle(apprasial, vehicle, vehicleProps)
					end
				else
				...
				exports['mythic_notify']:DoHudText('inform', _U('cannot_store_vehicle'), 3000, { ['background-color'] = '#22b1e6', ['color'] = '#ffffff' })
			end
		end, vehicleProps)
	else
		--ESX.ShowNotification(_U('no_vehicle_to_enter'))
		exports['mythic_notify']:DoHudText('inform', _U('no_vehicle_to_enter'), 3000, { ['background-color'] = '#22b1e6', ['color'] = '#ffffff' })
	end
end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_car', {
			...
			ESX.TriggerServerCallback('esx_advancedgarage:checkMoneyCars', function(hasEnoughMoney)
				if hasEnoughMoney then
					...
					exports['mythic_notify']:DoHudText('inform', _U('not_enough_money'), 3000, { ['background-color'] = '#22b1e6', ['color'] = '#ffffff' })
				end
			end)
		end, function(data, menu)
			menu.close()
...
-- Pound Owned Sherifing Menu
function ReturnOwnedSherifingMenu()
	ESX.TriggerServerCallback('esx_advancedgarage:getOutOwnedSherifingCars', function(ownedSherifingCars)
		local elements = {}
		...
		for _,v in pairs(ownedSherifingCars) do
			if Config.UseVehicleNamesLua then
				local hashVehicule = v.model
				local aheadVehName = GetDisplayNameFromVehicleModel(hashVehicule)
				local vehicleName  = GetLabelText(aheadVehName)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_sherifing', {
			title    = _U('pound_sheriff'),
			align    = 'top-right',
			elements = elements
		}, function(data, menu)
			ESX.TriggerServerCallback('esx_advancedgarage:checkMoneySherifing', function(hasEnoughMoney)
				if hasEnoughMoney then
					TriggerServerEvent('esx_advancedgarage:paySherifing')
					SpawnPoundedVehicle(data.current.value, data.current.value.plate)
				else
					--ESX.ShowNotification(_U('not_enough_money'))
					exports['mythic_notify']:DoHudText('inform', _U('not_enough_money'), 3000, { ['background-color'] = '#22b1e6', ['color'] = '#ffffff' })
...
function RepairVehicle(apprasial, vehicle, vehicleProps)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'delete_menu', {
		...
		if data.current.value == 'yes' then
			...
			vehicleProps.bodyHealth = 0 -- must be a decimal value!!!
			vehicleProps.engineHealth = 1000
			StoreVehicle(vehicle, vehicleProps)
		elseif data.current.value == 'no' then
			--ESX.ShowNotification(_U('visit_mechanic'))
			exports['mythic_notify']:DoHudText('inform', _U('visit_mechanic'), 3000, { ['background-color'] = '#22b1e6', ['color'] = '#ffffff' })
		end
	end, function(data, menu)
		menu.close()
	end)
...
function StoreVehicle(vehicle, vehicleProps)
	...
	exports['mythic_notify']:DoHudText('inform', _U('vehicle_in_garage'), 3000, { ['background-color'] = '#22b1e6', ['color'] = '#ffffff' })
end
...
function SpawnVehicle(vehicle, plate, gps)
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			...
			if string.gsub(GetVehicleNumberPlateText(vehicle),'^%s*(.-)%s*$', '%1') == string.gsub(plate,'^%s*(.-)%s*$', '%1') then
				...
				exports['mythic_notify']:DoHudText('error', 'Ez az autó már kint van az utcákon!', 3000, { ['background-color'] = '#ff0000', ['color'] = '#ffffff' })
	...
	if ESX.Game.IsSpawnPointClear(vector3(this_Garage.SpawnPoint.x, this_Garage.SpawnPoint.y, this_Garage.SpawnPoint.z), 3.0) then 
		...
		exports['mythic_notify']:DoHudText('inform', 'Útban van egy jármű!', 3000, { ['background-color'] = '#22b1e6', ['color'] = '#ffffff' })
...
function SpawnPoundedVehicle(vehicle, plate)
	...
	if ESX.Game.IsSpawnPointClear(vector3(this_Garage.SpawnPoint.x, this_Garage.SpawnPoint.y, this_Garage.SpawnPoint.z), 3.0) then 
		...
		--ESX.ShowNotification("Please move the vehicle that is in the way.")
		exports['mythic_notify']:DoHudText('inform', 'Útban van egy autó!', 3000, { ['background-color'] = '#22b1e6', ['color'] = '#ffffff' })
...
AddEventHandler('esx_advancedgarage:hasEnteredMarker', function(zone)
	...
	elseif zone == 'taxing_pound_point' then
		...
		CurrentActionMsg  = _U('press_to_impound')
		CurrentActionData = {}
	elseif zone == 'sherifing_pound_point' then
		CurrentAction     = 'sherifing_pound_point'
		CurrentActionMsg  = _U('press_to_impound')
		CurrentActionData = {}
	...
	while true do
		...
		if Config.UseCarGarages then
			...
			for k,v in pairs(Config.CarGarages) do
				...
				if (GetDistanceBetweenCoords(coords, v.GaragePoint.x, v.GaragePoint.y, v.GaragePoint.z, true) <= Config.MarkerDistance) then
				...
				if(GetDistanceBetweenCoords(coords, v.DeletePoint.x, v.DeletePoint.y, v.DeletePoint.z, true) <= Config.MarkerDistance) then
			...
			if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'sheriff' then
				for k,v in pairs(Config.SheriffPounds) do
					if (GetDistanceBetweenCoords(coords, v.PoundPoint.x, v.PoundPoint.y, v.PoundPoint.z, true) <= Config.MarkerDistance) then
						isInMarker  = true
						this_Garage = v
						currentZone = 'sherifing_pound_point'
					end
				end
			end
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif CurrentAction == 'sherifing_pound_point' then
					OpenMenuGarage('sherifing_pound_point')
